...
public:
    ...
    T get(int index) const {
        if (!isValidIndex(index)) {
            cout << "[DynamicArray] get(" << index << ") out of range. size=" << size_ << "\n";
            return T{};
        }
        return data_[index];
    }
    ...
    void set(int index, T value) {
        if (!isValidIndex(index)) {
            cout << "[DynamicArray] set(" << index << ") out of range. size=" << size_ << "\n";
            return;
        }
        data_[index] = value;
    }
    ...
    void insert(int index, T value) {
        if (index < 0 || index > size_) {
            cout << "[DynamicArray] insert(" << index << ") out of range. size=" << size_ << "\n";
            return;
        }
        if (size_ == capacity_) resize(capacity_ * 2);
    ...
    void remove(int index) {
        if (!isValidIndex(index)) {
            cout << "[DynamicArray] remove(" << index << ") out of range. size=" << size_ << "\n";
            return;
        }
